...
fn init_session(
    ...
    // NOTE (vendored patch): ort rc.13 types builder errors, so map them to
    // KokoroError::Config instead of relying on #[from] conversion.
    fn berr<E: std::fmt::Debug>(e: E) -> KokoroError {
        KokoroError::Config(format!("{e:?}"))
    }
    ...
    if write_cache {
        ...
        builder = builder.with_optimized_model_path(cache).map_err(berr)?;
    }
